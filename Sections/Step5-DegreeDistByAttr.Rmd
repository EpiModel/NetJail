...
for (i in seq_along(floors)){
  ...
  g <- (ggplot(temp,aes(x=Date)) + 
    ...
    xlab("Date") + ylab("Degree") + ylim(NA, 16))
  assign(paste0("f", i), g)
}
...
cat('\n####', 'Mean Degree Averaged Across All Days (by Attribute)', '\n')
par(mfrow = c(1,1))
ggplot(cOverallMeanDeg_floor,aes(x=floor)) + 
  geom_boxplot(aes(lower=(OverallMeanDeg-sd), upper=(OverallMeanDeg+sd), 
               ...
               stat="identity") + 
  ggtitle("Mean degree (across all days) by floor") +
...
ggplot(cOverallMeanDeg_race,aes(x=race)) + 
  geom_boxplot(aes(lower=(OverallMeanDeg-sd), upper=(OverallMeanDeg+sd), 
               ...
               stat="identity") + 
  ggtitle("Mean degree (across all days) by race") +
...
ggplot(cOverallMeanDeg_age,aes(x=age)) + 
  geom_boxplot(aes(lower=(OverallMeanDeg-sd), upper=(OverallMeanDeg+sd), 
               ...
               stat="identity") + 
  ggtitle("Mean degree (across all days) by age") +
...
cSubsetMeanDeg_floor <- temp %>% group_by(floor) %>% 
  summarise(SubsetMeanDeg = mean(MeanDeg), sd = sd(MeanDeg),
            min = min(MeanDeg), max = max(MeanDeg))
remove(temp)
cat('\n####', 'Mean Degree Averaged Across 01/11 - 01/21 (by Attribute)', '\n')
par(mfrow = c(1,1))
ggplot(cSubsetMeanDeg_floor,aes(x=floor)) + 
  geom_boxplot(aes(lower=(SubsetMeanDeg-sd), upper=(SubsetMeanDeg+sd), 
                   middle=SubsetMeanDeg, ymin=min, ymax=max, group = floor), 
               stat="identity") + 
  ggtitle("Mean degree (across daily subset) by floor") +
  xlab("Floor") + ylab("Mean Degree")
...
cSubsetMeanDeg_race <- temp %>% group_by(race) %>% 
  summarise(SubsetMeanDeg = mean(MeanDeg), sd = sd(MeanDeg),
            min = min(MeanDeg), max = max(MeanDeg))
remove(temp)
ggplot(cSubsetMeanDeg_race,aes(x=race)) + 
  geom_boxplot(aes(lower=(SubsetMeanDeg-sd), upper=(SubsetMeanDeg+sd), 
                   middle=SubsetMeanDeg, ymin=min, ymax=max, group = race), 
               stat="identity") + 
  ggtitle("Mean degree (across daily subset) by race") +
  xlab("Race") + ylab("Mean Degree")
...
remove(temp)
par(mfrow = c(1,1))
ggplot(cSubsetMeanDeg_age,aes(x=age)) + 
  geom_boxplot(aes(lower=(SubsetMeanDeg-sd), upper=(SubsetMeanDeg+sd), 
                   middle=SubsetMeanDeg, ymin=min, ymax=max, group = age), 
               stat="identity") + 
  ggtitle("Mean degree (across daily subset) by age") +
  xlab("Age") + ylab("Mean Degree")
cat('\n')
```
