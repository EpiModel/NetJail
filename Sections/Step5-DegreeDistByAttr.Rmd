...
par(mfrow = c(2,2))
floors <- 1:7
max.y <- round(max(cMeanDeg_floor$MeanDeg), 1)
min.y <- floor(min(cMeanDeg_floor$MeanDeg) * 10) / 10
...
max.y <- round(max(cMeanDeg_race$MeanDeg), 1)
min.y <- floor(min(cMeanDeg_race$MeanDeg) * 10) / 10
par(mfrow = c(2,2))
...
max.y <- round(max(cMeanDeg_age$MeanDeg), 1)
min.y <- floor(min(cMeanDeg_age$MeanDeg) * 10) / 10
par(mfrow = c(2,2))
plot.new()
...
remove(i, j, races, floors, days, ages)
```
