...
#print(knitr::kable(cLocDurations[cLocDurations$category == 
                                       #"Overall", ]))
print(knitr::kable(cLocDurations))
...
print(knitr::kable(bLocDurations))
...
durations <- merge(durations, cDailyLoc_full, by = c("id", "Date"))
...
durations3 <- durations %>% group_by(startCensored, endCensored, Race.x) %>% 
  summarise(meanDur = mean(duration), numSpells = n(), 
            meanNumLocs = mean(numLocs), .groups = "drop_last")
durations3$Race.x <- paste("Race", durations3$Race.x)
names(durations3)[names(durations3) == 'Race.x'] <- 'category'

#Find mean active spell duration by age
durations4 <- durations %>% 
  group_by(startCensored, endCensored, InitialAge.x) %>% 
  summarise(meanDur = mean(duration), numSpells = n(), 
            meanNumLocs = mean(numLocs), .groups = "drop_last")
durations4$InitialAge.x <- paste("Age", durations4$InitialAge.x)
names(durations4)[names(durations4) == 'InitialAge.x'] <- 'category'
...
#print(knitr::kable(durationsSummary[durationsSummary$category == "Overall",
                                    #c("startCensored", "endCensored", "category", 
                                      #"meanDur", "meanNumLocs", 
                                      #"percentSpells")]))
print(knitr::kable(durationsSummary))
