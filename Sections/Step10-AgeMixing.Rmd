...
time1 <- time1 %>% group_by(head.age, tail.age) %>% summarise(numEdges = n())
time1.mat <- time1 %>% pivot_wider(names_from = tail.age, 
                                   values_from = numEdges)
cat('\n####', 'Age Mixing on ', as.character(firstDay), '(Numbers)', '\n')
...
timen <- timen %>% group_by(head.age, tail.age) %>%
  summarise(numEdges = n())
timen.mat <- timen %>%
  pivot_wider(names_from = tail.age, values_from = numEdges)
cat('\n####', 'Age Mixing on ', as.character(lastDay), '(Numbers)', '\n')
