...
for (i in seq_len(nrow(dates))){
  ...
  if (i == 1){
    ...
    bDegDists_floor <- merge(bDegDists_floor, dist_f, by = c("floor", "degree"),
                             all = TRUE)
...
for (j in seq_along(floors)) {
  for (i in seq_along(days)) {
    barplot(bDegDists_floor[bDegDists_floor$floor == floors[j], ][, (days[i] + 2)],
        names.arg = bDegDists_floor[bDegDists_floor$floor == floors[j], ]$degree,
...
for (j in seq_along(races)) {
  for (i in seq_along(days)) {
    barplot(bDegDists_race[bDegDists_race$race == races[j], ][, (days[i] + 2)],
        names.arg = bDegDists_race[bDegDists_race$race == races[j], ]$degree,
        xlab = "Degree", ylab = "Frequency",
...
par(mfrow = c(2,2))
floors <- 1:7
max.y <- round(max(bMeanDeg_floor$MeanDeg), 1)
...
for (i in seq_along(floors)){
  ...
  g <- (ggplot(temp,aes(x=Date)) + 
    geom_boxplot(aes(lower=(MeanDeg-SD), upper=(MeanDeg+SD), middle=MeanDeg, 
...
max.y <- round(max(bMeanDeg_race$MeanDeg), 1)
...
max.y <- round(max(bMeanDeg_age$MeanDeg), 1)
...
cat('\n####', 
    'Block-Level Mean Degree Averaged Across Selected Timespan (by Attribute)', 
    '\n')
par(mfrow = c(1,1))
ggplot(bOverallMeanDeg_floor,aes(x=floor)) + 
...
bOverallMeanDeg_race <- bMeanDeg_race %>% group_by(race) %>% 
  summarise(OverallMeanDeg = mean(MeanDeg), sd = sd(MeanDeg), 
            min = min(MeanDeg), max = max(MeanDeg))
par(mfrow = c(1,1))
ggplot(bOverallMeanDeg_race,aes(x=race)) + 
  geom_boxplot(aes(lower=(OverallMeanDeg-sd), upper=(OverallMeanDeg+sd), 
...
bOverallMeanDeg_age <- bMeanDeg_age %>% group_by(age) %>% 
  summarise(OverallMeanDeg = mean(MeanDeg), sd = sd(MeanDeg),
            min = min(MeanDeg), max = max(MeanDeg))
par(mfrow = c(1,1))
ggplot(bOverallMeanDeg_age,aes(x=age)) + 
  geom_boxplot(aes(lower=(OverallMeanDeg-sd), upper=(OverallMeanDeg+sd), 
