...
LoadPrevDynNWObj <- TRUE #Set to TRUE iff timeframe is from 10/27 to 02/04
...
`cOverallMeanDeg` is a simple, un-weighted average of these mean degree values (each day is weighted equally, without regard for changing network size.)
...
## Step 10: Calculate and visualize age mixing matrix

To analyze the age mixing in our network, we begin by creating a data frame, `doubledEdges`, that contains every edge in `cEdges` twice: once with the node with the smaller `id` as the head, and once with it as the tail. We then merge in the age (broken into 10-year categories) of the head and the tail of each edge. 
