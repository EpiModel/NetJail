...
EstimateLocDurations <- TRUE
EstimateDurations <- TRUE
AnalyseShortVsLong <- TRUE
```
...
## Step 3: Create a dynamic network object for cell-level network
...
Then we're ready to create our cell-level `networkDynamic` object. We use the `cLocs` data frame we created in Step 2 to create a dynamic (TEA) node attribute called `location`. We also set age, race, and gender as (static) vertex attributes. (Each person's age is calculated as of the first day that they appear in the data. Some of the residents may have a birthday during the time period we're considering, but we ignore that.) Finally, we activate nodes based on the data in `activeDays`. 

```{r, child = 'Sections/Step3-DynNWObject.Rmd', include = FALSE}
```

## Step 4: Calculate overall degree distribution for cell-level network
...
## Step 5: Calculate degree distribution by attribute for cell-level network
...
## Step 6: Estimate rate of cell changes within jail
...
We also perform these same calculations broken down by race, age, and floor (i.e., the floor of the cell that is vacated). 
...
_What percentage of location changes are to a new floor vs. a new block within the same floor vs. a new cell within the same block? Essentially need a probability matrix?_
...
_Want the average time spent in a particular location for short-term vs. long-term residents, broken down by location spell censoring status. (e.g., among long-term residents, what percentage of location spells are uncensored, and how long are uncensored location spells on average, etc.)_

```{r, child = 'Sections/Step8-LongVsShortStays.Rmd', echo = FALSE, message = FALSE, warning=FALSE}
```

## Step 9: Create animations of cell-level edges within a single block
...
```{r, child = 'Sections/Step9-Animations.Rmd', results = 'asis', echo = FALSE, message = FALSE, warning = FALSE}
```

## Step 10: Calculate and visualize age mixing matrix (for cell-level network)
...
```{r, child = 'Sections/Step10-AgeMixing.Rmd', results = 'asis', echo = FALSE, message = FALSE, warning = FALSE}
```

## Step 11: Create edge list and dynamic network object for block-level network

To create the edge list of block-level edges, we use the same approach we used for the cell-level network in Step 1 (and Step 3). We store the block-level edge list in `bEdges`.

Because the block-level network is huge, it is impractical to create a block-level `networkDynamic` object here, so we load in a previously created object instead. Currently, a block-level `networkDynamic` object (`bDynNW`) is only available for the 10/27 - 02/04 time frame, so if a different time frame has been specified, then degree distributions, etc. will not be calculated for the block-level network. (Like `cDynNW`, `bDynNW` has a dynamic (TEA) node attribute called `location` (crated from `cLocs`) that tells us what cell (NOT block) the node is in when known/assumed.)

Once `bDynNW`  has been loaded, we set age, race, and gender as (static) vertex attributes and activate nodes based on `activeDays` as we did in Step 3 for the cell-level network.

```{r, child = 'Sections/Step11-BlockLevelNWSetup.Rmd', include = FALSE}
```

## Step 12: Calculate overall degree distribution for block-level network

To calculate the block-level degree distribution over time, we adapt the same approach that we used in Step 4 to calculate the cell-level degree distribution. 
...
```{r, child = 'Sections/Step12-BlockLevelDegreeDist.Rmd', echo = FALSE, message = FALSE, warning = FALSE}
```

## Step 13: Calculate degree distribution by attribute for block-level network

In Step 13, we perform the same operations as in Step 12, but broken down by race, age, and floor. As in Step 5, we break age into 10-year age categories and divide race into Black, White, and Other.

```{r, child = 'Sections/Step13-BlockDegreeDistByAttr.Rmd', results = 'asis', echo = FALSE, message = FALSE, warning = FALSE}
```
